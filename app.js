...
// ── El Veedor — Mesa de Luz ─────────────────────────────────────

function initVeedor() {
const sw      = document.getElementById('veedorSwitch');
const block   = document.getElementById('orderFormBlock');
const resultEl = document.getElementById('veedorResult');
if (!sw) return;

sw.addEventListener('change', () => {
  veedorActive = sw.checked;
  if (block) block.classList.toggle('mesa-de-luz', veedorActive);
  if (!veedorActive) {
    if (resultEl) { resultEl.style.display = 'none'; resultEl.innerHTML = ''; }
  } else if (selectedFile) {
    const medida = document.querySelector('select[name="size"]')?.value || '';
    analyzeWithVeedor(selectedFile, medida);
  }
});

document.querySelector('select[name="size"]')?.addEventListener('change', () => {
  if (veedorActive && selectedFile) {
    const medida = document.querySelector('select[name="size"]')?.value || '';
    analyzeWithVeedor(selectedFile, medida);
  }
});
...
async function analyzeWithVeedor(file, medida) {
const resultEl = document.getElementById('veedorResult');
if (!resultEl) return;
resultEl.style.display = 'block';
resultEl.innerHTML = '<div class="veedor-progress">[ Leyendo archivo... ]</div>';

let base64;
try {
  const raw = await toBase64(file);
  base64 = String(raw).includes(',') ? String(raw).split(',')[1] : String(raw);
} catch {
  resultEl.innerHTML = '<div class="veedor-progress">[ Error al leer el archivo ]</div>';
  return;
}

resultEl.innerHTML = '<div class="veedor-progress">[ Leyendo archivo... ]<br>[ Verificando escala... ]</div>';
...
try {
  const response = await fetch(APPS_SCRIPT_URL, {
    method:  'POST',
    headers: { 'Content-Type': 'text/plain;charset=utf-8' },
    body:    JSON.stringify({ action: 'analizar_plano', sessionId: sid, fileData: base64, medida: medida || '' }),
    signal:  controller.signal,
  });
  clearTimeout(timeoutId);
  const data = JSON.parse(await response.text());
  if (!data.ok) throw new Error(data.error || 'No se pudo analizar el plano.');
  renderVeedorResult(resultEl, data.analisis);
} catch (err) {
  clearTimeout(timeoutId);
  const msg = err.name === 'AbortError' ? 'Tiempo de espera agotado.' : (err.message || 'Error de análisis.');
  resultEl.innerHTML = `<div class="veedor-progress">[ Error: ${msg} ]</div>`;
}
}

function renderVeedorResult(el, a) {
const estadoKey   = (a.estado || '').toUpperCase();
const estadoClass = estadoKey === 'OK' ? 'ok' : estadoKey === 'ERROR' ? 'error' : 'warn';
const estadoDot   = estadoClass === 'ok' ? '●' : estadoClass === 'error' ? '✕' : '▲';
const hallazgos   = Array.isArray(a.hallazgos)
  ? a.hallazgos.map(h => `— ${h}`).join('<br>')
  : (a.hallazgos || '—');

el.innerHTML = `
  <div class="veedor-estado ${estadoClass}">${estadoDot} ${a.estado || '—'}</div>
  <div class="veedor-field">
    <span class="veedor-field-label">Escala detectada</span>
    <span class="veedor-escala">${a.escala_detectada || '—'}</span>
  </div>
  <div class="veedor-field">
    <span class="veedor-field-label">Hallazgos</span>
    <span class="veedor-field-value">${hallazgos}</span>
  </div>
  <div class="veedor-field">
    <span class="veedor-field-label">Sugerencia</span>
    <span class="veedor-field-value">${a.sugerencia || '—'}</span>
  </div>
  <div class="veedor-disclaimer">
    <strong>Verificación Técnica Inteligente (Mesa de Luz Beta)</strong>
    Esta herramienta utiliza inteligencia artificial para ofrecer una capa adicional de seguridad antes de la impresión, analizando la coherencia de escalas y proporciones.<br><br>
    Nota: La IA es un asistente de apoyo y puede cometer errores u omitir inconsistencias. El profesional responsable del archivo es el único encargado de verificar la exactitud de las medidas y escalas finales. En Bruto Estudio no asume responsabilidad legal por errores en la documentación técnica provista por el cliente.
  </div>`;
}
...
window.location.replace(data.must_change_password ? 'change-password.html' : 'panel.html');
...
if (!data.ok) {
// Si debe cambiar contraseña, redirigir sin borrar sesión
if (data.must_change_password) { window.location.replace('change-password.html'); return null; }
clearSession();
...
selectedFile = null;
updatePriceDisplay();
const vr = document.getElementById('veedorResult');
...
function initClock() {
  ...
  function tick() {
    ...
    const date = now.toLocaleDateString('es-AR', { timeZone: tz, weekday: 'long', day: 'numeric', month: 'long' });
...
async function initEfemeride() {
  const tituloEl = document.getElementById('efemeriTitulo');
  const descEl   = document.getElementById('efemeriDesc');
  if (!tituloEl || !descEl) return;

  try {
    const resp = await fetch('/efemerides.json');
    if (!resp.ok) return;
    const data = await resp.json();

    const tz  = 'America/Argentina/Buenos_Aires';
    const now = new Date();
    const day = parseInt(now.toLocaleDateString('es-AR', { timeZone: tz, day: 'numeric' }), 10);
    const monthIdx = parseInt(now.toLocaleDateString('en-US', { timeZone: tz, month: 'numeric' }), 10) - 1;
    const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const mes = meses[monthIdx];

    const entry = data.find(e => e.día === day && e.mes === mes);
    if (entry) {
      tituloEl.textContent = entry.título || entry.titulo || '—';
      descEl.textContent   = entry.descripción || entry.descripcion || '—';
    }
  } catch (_) {}
}

function initPanelClocks() {
  const pairs = [
    { date: document.getElementById('panelTsDate'), time: document.getElementById('panelTsTime') }
  ...
  function tick() {
    ...
    const date = now.toLocaleDateString('es-AR', { timeZone: tz, weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
    const dateStr = date.charAt(0).toUpperCase() + date.slice(1);
    pairs.forEach(p => { p.date.textContent = dateStr; p.time.textContent = time; });
  }
...
// Toggle preflight embebido
const pfToggleBtn = document.getElementById('pfToggleBtn');
const dashGrid    = document.querySelector('.dashboard-grid');
const pfEmbedded  = document.getElementById('pfEmbedded');
if (pfToggleBtn) {
  const pfLabel = pfToggleBtn.querySelector('.nav-pf-label');
  pfToggleBtn.addEventListener('click', () => {
    const isOn = pfToggleBtn.classList.toggle('nav-pf-toggle--on');
    pfToggleBtn.setAttribute('aria-pressed', String(isOn));
    document.body.classList.toggle('bruto-beta', isOn);
    if (pfLabel)    pfLabel.textContent      = isOn ? 'BETA'  : 'BRUTO';
    if (dashGrid)   dashGrid.style.display   = isOn ? 'none'  : '';
    if (pfEmbedded) pfEmbedded.style.display = isOn ? 'block' : 'none';
  });
}
