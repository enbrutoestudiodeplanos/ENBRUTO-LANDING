...
const MOVIES = [
  { id: 1,  title: 'The Fountainhead',                year: 1949, tmdbId: 19690,    posterFallback: 'https://m.media-amazon.com/images/M/MV5BYTk4ZGYzNTgtZmNiNy00NWRkLTlhMmMtMTk3ZTM1MzFhM2I3XkEyXkFqcGdeQXVyNjc1NTYyMjg@._V1_SX300.jpg', summary: 'Arquitectura como ideología, autoría, ego y conflicto con el cliente. Un arquitecto se niega a comprometer su visión.', trailerQuery: 'The+Fountainhead+1949+trailer' },
  ...
  { id: 4,  title: 'Columbus',                          year: 2017, tmdbId: 411001,   summary: 'Silencio, geometría, contemplación, modernismo. Dos extraños exploran la arquitectura de Columbus, Indiana.', trailerQuery: 'Columbus+2017+trailer' },
  ...
  { id: 11, title: 'The Last Black Man in San Francisco', year: 2019, tmdbId: 475943,   summary: 'Ciudad, vivienda, gentrificación, pertenencia. Un joven busca reclamar la casa que su abuelo construyó.', trailerQuery: 'Last+Black+Man+San+Francisco+trailer' },
  ...
  { id: 17, title: 'The International',                 year: 2009, tmdbId: 12171,    summary: 'Lenguaje arquitectónico y espacial. Un agente investiga un banco corrupto a través del diseño y la arquitectura.', trailerQuery: 'The+International+trailer' },
...
function setupEventListeners() {
  ...
  list.addEventListener('click', (e) => {
    ...
    // Si clickearon el custom checkbox box (span) o el label/wrap
    if (checkboxWrap && !checkbox) {
      e.preventDefault();
      e.stopPropagation();
      const input = checkboxWrap.querySelector('input[type="checkbox"]');
      const movieId = parseInt(input.dataset.movieId);
      // Toggle manualmente
      input.checked = !input.checked;
      toggleWatched(movieId, input.checked);
      return;
    }
    
    // Si clickearon directamente el input nativo
    if (checkbox) {
      e.stopPropagation();
      const movieId = parseInt(checkbox.dataset.movieId);
      // El checked ya se actualizó por el click nativo
      setTimeout(() => toggleWatched(movieId, checkbox.checked), 0);
      return;
    }
  });
...
function toggleWatched(movieId, checked) {
  // Si no viene checked, toggle el estado actual
  if (typeof checked === 'undefined') {
    checked = !watchedState[movieId];
  }
  
  watchedState[movieId] = checked;
  saveState();
...
async function openMovieDetail(movieId) {
  ...
  // Obtener póster (con fallback del dataset si existe)
  let posterUrl = await fetchPoster(movie.tmdbId);
  if ((!posterUrl || posterUrl.includes('placehold.co')) && movie.posterFallback) {
    posterUrl = movie.posterFallback;
  }
  
  // Trailer: link de búsqueda en YouTube (el embed de search no funciona sin video ID)
  const trailerSearchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(movie.trailerQuery.replace(/\+/g, ' '))}`;
  ...
  content.innerHTML = `
    ...
    <a class="trailer-link" href="${trailerSearchUrl}" target="_blank" rel="noopener noreferrer">
...
async function fetchPoster(tmdbId) {
  ...
  // Fallback
  return `https://placehold.co/300x450/111111/555555?text=Sin+poster`;
}
