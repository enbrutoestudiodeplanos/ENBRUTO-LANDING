// reset-password.js — Restablece contraseña vía Apps Script (sin Supabase)

const RESET_APPS_URL = 'https://script.google.com/macros/s/AKfycbyHjHN5w6-Bff7Wk88-ac4qRGD_fr_Vpqk96dc8dv4KjCqpzgfJdZG7cbsW9qE3BURbTw/exec';
...
async function appsPost(payload) {
  ...
  try {
    const res  = await fetch(RESET_APPS_URL, {
      method:  'POST',
  ...
  document.getElementById('resetForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        ...
        if (btn) { btn.disabled = false; btn.innerHTML = '<span>Guardar contraseña</span>'; }
        return;
      }
      show('successView');
    } catch {
